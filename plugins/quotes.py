			...
			if (len(tokens) > 4):
				if (tokens[4] == "add"):
					if (len(tokens) > 5):
						...
						parent.s.send(PRIVMSG(parent.channel, "Quote added.", 0))
					else:
						parent.s.send(PRIVMSG(parent.channel, ".wedr add <MESSAGE> - Add MESSAGE to Quotes List.", 0))
				elif (tokens[4].isnumeric()):
					try:
						quoteIndex = int(tokens[4])
						if (quoteIndex > len(quotesList) or quoteIndex <= 0):
							print("Index is invalid.")
							parent.s.send(PRIVMSG(parent.channel, "Invalid quote index. Enter number from 1 to %d." % (len(quotesList)), 0))
						else:
							print("Showing quote #%s" % (str(quotesList[quoteIndex - 1])))
							parent.s.send(PRIVMSG(parent.channel, "Quote #%s: %s" % (str(quoteIndex), str(quotesList[quoteIndex - 1])), 0))
					except ValueError:
					...
					if (len(tokens) > 5 and tokens[5].isnumeric()):
						try:
							...
							if (quoteIndex > len(quotesList) or quoteIndex <= 0):
								...
								parent.s.send(PRIVMSG(parent.channel, "Quote #%s has been removed." % (tokens[5]), 0))
						except Exception:
							print("Quote #%s does not exist." % (tokens[5]))
							parent.s.send(PRIVMSG(parent.channel, "Quote #%s does not exist." % (tokens[5]), 0))
					else:
						if (len(quotesList) == 0):
							print("Quotes List empty.")
							parent.s.send(PRIVMSG(parent.channel, ".wedr remove <INDEX> - Remove MESSAGE matching INDEX from Quotes List. Quotes List is currently empty.", 0))
						else:
							print("Index not given.")
							parent.s.send(PRIVMSG(parent.channel, ".wedr remove <INDEX> - Remove MESSAGE matching INDEX (range: 1 to %d) from Quotes List." % (len(quotesList)), 0))
				elif (tokens[4] == "capacity" or tokens[4] == "size"):
					print("Showing quotes list size.")
					parent.s.send(PRIVMSG(parent.channel, "Quotes List size : %d" % (len(quotesList)), 0))
				elif (tokens[4] == "help"):
					...
					parent.s.send(PRIVMSG(parent.channel, "Quotes List is saved.", 0))
				elif (tokens[4] == "load"):
					load()
					print("Quotes List loaded.")
					parent.s.send(PRIVMSG(parent.channel, "Quotes List is loaded.", 0))
			else:
