			...
			if (len(messageTokens) >= 2):
				if (messageTokens[1] == "add"):
					if (len(messageTokens) > 2):
						...
						self.wedrs.append("[%s] %s" % (user, tempString))
					...
					try:
						...
						if (quoteIndex > len(self.wedrs) or quoteIndex <= 0):
							print("Index is invalid.")
							self.sendMessage(self.channel, "Invalid quote index. Enter number from 1 to current size of Quotes List.")
						else:
							print("Showing quote #%s" % (str(self.wedrs[quoteIndex - 1])))
							self.sendMessage(self.channel, "Quote #%s: %s" % (str(quoteIndex), str(self.wedrs[quoteIndex - 1])))
					except ValueError:
					...
					try:
						...
						if (quoteIndex > len(self.wedrs) or quoteIndex <= 0):
							...
							self.wedrs.remove(self.wedrs[quoteIndex - 1])
					...
					self.sendMessage(self.channel, "Quotes List size : %s" % (str(len(self.wedrs))))
